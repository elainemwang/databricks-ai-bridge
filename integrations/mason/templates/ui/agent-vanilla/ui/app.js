const elements = {
  ...
  hitlControls: document.querySelector("#hitl-controls"),
...
function setBusy(busy, label = "Working") {
  ...
  elements.resumeSession.disabled = busy || !state.config?.hitl || !state.config?.session.durable;
  elements.rejectSession.disabled = busy || !state.config?.hitl || !state.config?.session.durable;
...
async function loadConfig() {
  try {
    ...
    // Human-in-the-loop controls only apply to frameworks that gate tools on approval; this
    // template does not, so config.hitl is false and the paused-HITL controls stay hidden.
    elements.hitlControls.hidden = !config.hitl;
    elements.resumeSession.disabled = state.busy || !config.hitl || !config.session.durable;
    elements.rejectSession.disabled = state.busy || !config.hitl || !config.session.durable;
    ...
    elements.sessionStoreLabel.textContent = config.session.managed
      ? `${config.session.store} · actor ${config.session.actor} · the Apps routing cookie keys the transcript.`
      : config.session.history
        ? "Messages load from the in-process message list for the current routing cookie."
